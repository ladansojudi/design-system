...
function TokenRow({ sw }: { sw: Swatch }) {
  const copy = useCopy();
  ...
  return (
    <tr className="border-t border-s4e-neutral-divider-10">
      ...
      <td className="py-2 px-3 align-middle">
        <span className="font-mono text-[11px] text-s4e-text-secondary">
          rgb({sw.rgb.join(", ")})
        </span>
      </td>
      <td className="py-2 px-3 align-middle">
        ...
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="text-left">
              {["", "Token", "Hex", "RGB", "vs White", "vs Black", "Font"].map((h, i) => (
                <th
                  key={i}
